...
| Workflow | Role |
|----------|------|
| `ci.yml` / `ci-dev.yml` | `bun run verify` on PRs |
| `deploy.yml` | Verify, upload, promote on `main`; staging upload on `dev` |
| `infra-plan.yml` | `tofu plan` when `infra/tofu/**` or related docs change |
| `push-worker-secrets.yml` | Manual sync from Actions secrets to the Secrets Store |
| `token-expiry-check.yml` | Monthly token check |
| `rollback.yml` | Promote an older Worker version |
