  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (a === "--from") from = argv[++i] ?? "";
      ...
      process.stdout.write(
        [
          "Usage: pnpm run env:copy-prod [--from staging] [--to production] [--secrets-file infra/secrets/worker-secrets.local.json]",
          "",
...
function readJson(path: string): WorkerSecrets {
  try {
    ...
    die(
      `Failed to read ${path}. Copy from infra/secrets/worker-secrets.example.json.\n${String(err)}`,
    );
  }
}
...
function main(): void {
  ...
  upsertEnvVar(
    ...
    must(blob.cloudflare_account_id, "cloudflare_account_id"),
  );
  upsertEnvVar(
    repo,
    to,
