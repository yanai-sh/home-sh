...
- **pnpm monorepo** — **SvelteKit 5 + `@sveltejs/adapter-cloudflare`** site as a Cloudflare Worker with Static Assets; `/api/contact`, **`/resume.pdf`** (Release proxy; **`RESUME_REPO_TOKEN`**), on the site Worker. Optional **`resume/`** submodule (Rust PDF). Toolchain: **Vite 8**, **svelte-check**, **Vitest**, **Velite**.
