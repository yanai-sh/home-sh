// Workspace telemetry — beacon on pagehide / visibility hidden. DNT + opt-out gate.
...
export function mountTelemetry(): TelemetryHandle | null {
  ...
  const tick = () => {
    ...
    if (now - lastSampleAt >= FRAME_SAMPLE_INTERVAL_MS) {
      ...
      const samples = beacon.frame_samples ?? [];
      if (samples.length < FRAME_SAMPLE_CAP) {
        samples.push({ t: Math.round(Date.now()), fps });
      }
      frames = 0;
      lastSampleAt = now;
    }
