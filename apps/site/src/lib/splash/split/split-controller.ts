...
export type SplitControllerElements = {
  root: HTMLElement;
  shell: HTMLElement;
...
export type SplitControllerCallbacks = {
  onProgress?: (progress: number, open: boolean) => void;
  onMode?: (mode: SiteMode, activeProject: string) => void;
};

type SplitControllerDeps = {
  elements: SplitControllerElements;
  reducedMotion: boolean;
  callbacks?: SplitControllerCallbacks;
};
...
function splashActiveFor(mode: SiteMode, activeProject: string): string {
  if (mode === "project") return activeProject || "projects";
  if (mode === "splash") return "splash";
  return mode;
}

export function createSplitController(deps: SplitControllerDeps): SplitController {
  const { elements, reducedMotion, callbacks } = deps;
  const {
    root,
    shell,
    paneDetail,
  ...
  const syncHtml = (): void => {
    const open = splitProgress > 0.004;
    root.dataset.splitOpen = open ? "true" : "false";
    if (!open) delete root.dataset.splitOpen;
    syncSplashHtmlState(root, {
      open,
      active: splashActiveFor(mode, activeProject),
      view: mode === "project" ? "project" : "dest",
      animating:
        root.classList.contains("is-split-animating") ||
        root.classList.contains("is-splash-animating"),
      siteMode: mode,
    });
    callbacks?.onProgress?.(splitProgress, open);
    callbacks?.onMode?.(mode, activeProject);
  };
  ...
  const animateSplit = (target: number, isClosing: boolean, onDone?: () => void): void => {
    const duration = reducedMotion ? 0 : isClosing ? SPLIT_CLOSE_MS : SPLIT_OPEN_MS;
