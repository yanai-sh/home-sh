...
- **PR — main** (`ci.yml`, workflow **`yanai-sh / PR — main`**): **`pull_request`** to **`main`** — matrix **`ubuntu-latest`** + **`macos-latest`** calling **`reusable-verify.yml`** (**`yanai-sh / Verify (reusable)`**). Required check contexts (include reusable job suffix): **`yanai-sh / verify (ubuntu-latest) / yanai-sh / verify — run`** and the macOS pair — see **`scripts/ruleset-protect-main.json`**; **`./scripts/gh-protect-main.sh`** defaults to **`yanai-sh/home-sh`**. Long-lived **`dev`**: run **`./scripts/gh-protect-dev.sh`** once (or apply **`scripts/ruleset-protect-dev.json`**) so GitHub **blocks deletion and force-push** on **`dev`** while still allowing normal pushes for staging deploy.
