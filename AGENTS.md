...
- **PR — main** (`ci.yml`, workflow **`yanai-sh / PR — main`**): **`pull_request`** to **`main`** — matrix **`ubuntu-latest`** + **`macos-latest`** calling **`reusable-verify.yml`** (**`yanai-sh / Verify (reusable)`**). Required check contexts (include reusable job suffix): **`yanai-sh / verify (ubuntu-latest) / yanai-sh / verify — run`** and the macOS pair — see **`scripts/ruleset-protect-main.json`**; **`./scripts/gh-protect-main.sh`** defaults to **`yanai-sh/home-sh`**. For long-lived **`dev`**, configure the same kind of thing in **GitHub → Settings → Rules → Rulesets** (target **`dev`**, enable **block branch deletion** and **block force pushes**, and do **not** require pull requests if you still want direct **`git push origin dev`** for Deploy).
