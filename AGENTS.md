...
- **Deploy** (`deploy.yml`, **`yanai-sh / Deploy`**): **`push`** to **`dev`** / **`main`** and **`workflow_dispatch`** (**`skip_smoke: 'true'`** skips staging smoke). Jobs: **`yanai-sh / deploy — publish`**, **`yanai-sh / deploy — version tag`** (**`main`** only), **`yanai-sh / deploy — GitHub release`** (**`main`** only), **`yanai-sh / deploy — smoke`**. No **`pull_request`** on deploy. Project-pinned wrangler 4.x; config from **`apps/site/dist/server/wrangler.json`** after build.
...
Two long-lived branches, two Workers: **`yanai-sh-staging`** on **`dev`**, **`yanai-sh`** on **`main`** (same `apps/site/wrangler.jsonc`; deploy passes **`--name yanai-sh-staging`** on **`dev`**). **Deploy** runs on **`push`** to **`dev`** or **`main`**, not on **`pull_request`** into **`dev`** (avoids uploading twice when a PR merges). **Staging** uploads use a Wrangler-only label (`dev-<run_id>`); **no git tags on `dev`**. **Production** pushes create a **patch** semver git tag (`vX.Y.(Z+1)`) + GitHub Release. For a deliberate **minor** / **major** without extra commits, run **Actions → Deploy → Run workflow** on **`main`** and set **`version_bump`** (default remains **patch** on normal pushes).
...
A production release is whatever lands on **`main`** (Deploy auto-tags the next **patch** `vX.Y.(Z+1)` on each push). Before **`dev` → `main`**: move **`CHANGELOG`** **`[Unreleased]`** into **`## [vX.Y.Z] - YYYY-MM-DD`**, leave a fresh **`[Unreleased]`** (align **`Z`** with the tag the workflow will create, or edit after merge). For a **minor/major** story release, use **workflow_dispatch** on **`main`** with **`version_bump`**, or add a chore commit and let the next **patch** land. Watch deploy: **`gh run watch`**. Smoke prod: **`bun run --cwd apps/site smoke`** with **`SMOKE_BASE_URL=https://yanai.sh`** when needed. If prod is wrong, **`ARCHITECTURE.md`** documents Worker rollback.
