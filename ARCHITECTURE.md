...
| **Splash ambient** | Production `/` uses a baked **paper-fog still** (`SplashAmbient`, `--splash-still`). Split-pane resume, contact, and projects on the same Worker. Contact: Turnstile + Resend in `lib/server/contact.ts`. |
