...
| **`/resume.pdf` on-request from GitHub Releases** | **`GET /resume.pdf`** lists **`yanai-sh/resume`**’s latest release, finds **`YanaiKlugman_CV_*.pdf`**, and streams it with a PAT from **Secrets Store** (`RESUME_REPO_TOKEN` / **`resume_repo_token`** in SOPS). No static asset in `dist/client/`. |
