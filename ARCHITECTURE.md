...
- **Secrets rotation gone wrong** — restore GitHub repository secrets + gitignored **`infra/secrets/worker-secrets.local.json`** / **`.dev.vars`**, re-run **Push Worker secrets** (or `bun run push-secrets`), and restore **`infra/tofu/terraform.tfvars`** + **`CLOUDFLARE_API_TOKEN`** if the API token rotated; then re-apply Tofu as needed.
