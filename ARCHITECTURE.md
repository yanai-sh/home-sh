...
| **Splash ambient** | Production `/` uses a baked **paper-fog still** (`SplashAmbient`, `--splash-still`). **SplashDeck** flyout carousel (resume · projects · contact) on the same Worker; archived canvas experiment at `/labs/splash-canvas`. Contact: Turnstile + Resend in `lib/server/contact.ts`. |
| **Vite+ + svelte-check + Velite**                                             | `vp check` (Oxlint, Oxfmt, tsgo) plus Svelte-specific checks; Velite for content collections at build time.                                                                                                                                                                                         |
| **Nub + Node 24**                                                              | Package manager and runtime; CI uses **`nub-install`** composite.                                                                                                                                                                                                   |
| **Geist + Space Grotesk**                                                       | Body and heading fonts via `@fontsource` in `global.css`.                                                                                                                                                                                                            |
...
| Decision                                                                                    | Rationale                                                                                                                                                                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Protected `main` + PRs from topic branches**                                              | Trunk flow: topic branch → PR → **`main`**. **CI:** **`pull_request`** to **`main`** or **`dev`** (`.github/workflows/ci.yml`, **`ubuntu-latest`** only). **`push`** to **`main`** does not re-run CI—**Deploy** already runs **`verify`** on that path.                                                                                                           |
...
| Decision                                  | Rationale                                                                         |
| ----------------------------------------- | --------------------------------------------------------------------------------- |
| **Vite+ + svelte-check + Vitest**         | `vp check` (Oxlint, Oxfmt, tsgo) plus Svelte-specific checks and unit tests—proportionate for a portfolio site. |
...
| ------------------------------------------ | ----------------------------------------------- | -------------------------------------------------------------------------------- |
| **Vite+ (`vp`)**                           | dev, check, build                               | Oxlint, Oxfmt, tsgo via `vp check`; Svelte via `svelte-check`                   |
| **Vitest**                                 | Unit/handler tests                              | `vitest.config.ts` avoids loading workerd in tests                               |
| **svelte-check**                           | Svelte/Kit template diagnostics               | Complements `vp check` for `.svelte` files                                       |
...
| **`main` push rejected**                | Expected when rulesets require PRs. Push a **topic branch**, open **PR → `main`**, merge when **`yanai-sh / verify / yanai-sh / verify — run`** is green (`ubuntu-latest`).     |
