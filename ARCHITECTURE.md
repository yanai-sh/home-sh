...
| **Splash WebGL field**                                                        | Visible demo: **`field-fluid.ts`** stable-fluids chrome (theme/split/contact uniforms), orchestrated by **`field.ts`** with a CSS still-frame fallback. **`POST /api/contact`** validation, Turnstile, Resend, and rate limits live in TypeScript (`lib/server/contact.ts`, `contact-validate.ts`). |
