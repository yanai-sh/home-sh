...
- Hono SSR serves durable HTML first.
...
- Security middleware with CSP, HSTS on HTTPS, and COOP/COEP scoped to
  `/workspace`.
- Local quality gate: `pnpm run verify`.
...
- Use pnpm commands from the repository root unless a package script requires a
  package directory.
...
- Run `pnpm run verify` before merge-sized commits. For docs-only changes,
...
resume/resume.toml (submodule pin)
  -> pnpm run sync:resume (scripts/sync-resume.ts)
...
- **`git submodule update --init --recursive`** is required on clone and in CI
  (`submodules: recursive` on checkout). **`pnpm run verify`** runs
...
- One **normalized** schema (`home-sh-resume-v1`) before any SSR render or
  bundle import.
...
- **`pnpm run sync:resume`** reads **`resume.toml`**, normalizes, validates with
...
- Velite **content collections** for non-resume structured content if/when
...
- Invalid **`resume.toml`** fails **`pnpm run sync:resume`** / **`verify`**.
...
- Audit `views/splash-layout.tsx`, `middleware/security.ts`, and client script loading for CSP.

Acceptance criteria:

- `pnpm run verify` passes.
- `pnpm run preview` serves the built artifact.
...
- Keep CI and deploy on the same `pnpm run verify` gate.
...
- A clean clone can run `pnpm install`, `pnpm run verify`, and `pnpm run build`.
...
- Return stable error codes that the contact form can display.
